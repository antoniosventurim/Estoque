<x-app.layout>
    ...
    <div class="space-y-4">
        ...
        @if ($success = session('success'))
            <x-app.card class="border-success/40 p-5" style="border-color: #34c45a55; background: #1a6b3522">
                <div class="mb-3 flex items-center gap-2.5">
                    ...
                    <span class="text-[15px] font-semibold text-mv-success">Saída registrada com sucesso</span>
                </div>
                <div class="flex flex-wrap items-start gap-x-8 gap-y-3">
                    ...
                    <div>
                        <p class="text-[13px] uppercase tracking-wider text-mv-text-muted">Quantidade retirada</p>
                        <p class="text-[14px] font-medium text-mv-text">{{ $success['qty'] }} un</p>
                    </div>
                    <div>
                        <p class="text-[13px] uppercase tracking-wider text-mv-text-muted">Estoque anterior</p>
                        <p class="text-[14px] font-medium text-mv-text">{{ $success['before'] }} un</p>
                    </div>
                    <div>
                        <p class="text-[13px] uppercase tracking-wider text-mv-text-muted">Estoque atual</p>
                        <p class="text-[14px] font-medium text-mv-text">{{ $success['after'] }} un</p>
                    </div>
                    <div>
                        <p class="text-[13px] uppercase tracking-wider text-mv-text-muted">Estoque mínimo</p>
                        <p class="text-[14px] font-medium text-mv-text">{{ $success['min_stock'] }} un</p>
                    </div>
                    <div>
            ...
            <x-app.card class="p-5">
                ...
                <div class="mb-4 grid grid-cols-2 gap-3">
                    <div class="rounded-lg border border-mv-border bg-mv-surface2 p-3">
                        <p class="text-[13px] uppercase tracking-wider text-mv-text-muted">Estoque atual</p>
                        <p id="saida-stock" data-value="{{ $product->stock }}" class="text-xl font-bold text-mv-text">{{ $product->stock }} <span class="text-[14px] font-normal text-mv-text-muted">un</span></p>
                    </div>
                    <div class="rounded-lg border border-mv-border bg-mv-surface2 p-3">
                        <p class="text-[13px] uppercase tracking-wider text-mv-text-muted">Estoque mínimo</p>
                ...
                <form method="POST" action="{{ route('saida.register') }}" id="saida-form">
                    @csrf
                    <input type="hidden" name="barcode" value="{{ $product->barcode }}">

                    <div class="grid gap-3 sm:grid-cols-2">
                        <div>
                            <label for="quantity" class="mb-1.5 block text-[14px] font-medium text-mv-text-secondary">Quantidade a ser retirada</label>
                            <input id="quantity" name="quantity" type="number" min="1" max="{{ $product->stock }}" value="1" class="w-full rounded-md border border-mv-border bg-mv-surface2 px-2.5 py-2 text-center text-[15px] font-semibold text-mv-text outline-none">
                        </div>
                        <div>
                            <label class="mb-1.5 block text-[14px] font-medium text-mv-text-secondary">Novo estoque</label>
                            <input id="saida-new-stock" type="text" value="{{ $product->stock - 1 }} un" disabled class="w-full cursor-not-allowed rounded-md border border-mv-border bg-mv-surface2/60 px-2.5 py-2 text-center text-[15px] font-semibold text-mv-success">
                        </div>
                    </div>
                    ...
                    <button type="submit" class="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-mv-accent py-3 text-sm font-semibold text-white hover:bg-mv-accent-hover">
                        <x-app.icon name="arrow-down" :size="16" /> Registrar Saída
                    </button>
                </form>
            </x-app.card>
        @endif
    ...
    <script>
        (function () {
            ...
            const current = Number(document.getElementById('saida-stock')?.dataset.value);
            if (!qty || !preview || Number.isNaN(current)) return;

            const render = () => {
                const q = Math.max(1, Number(qty.value) || 1);
                preview.value = (current - q) + ' un';
                preview.style.color = (current - q) < 0 ? '#f45a6c' : '';
            };
